    ...
    return (
        <View style={[styles.card, { backgroundColor: C.white, borderColor: C.border }]}>
            ...
            <View style={[styles.header, { borderBottomColor: C.border }]}>
                {isEditingTitle ? (
                    <TextInput
                        style={[styles.titleInput, { color: C.textDark }]}
                        value={titleText}
                    ...
                    <Text style={[styles.title, { color: C.textDark }]}>{checklist.title}</Text>
                )}
                    ...
                    <View key={item.id} style={styles.itemRow}>
                        <AnimatedTouchable onPress={() => onToggleItem(checklist.id, item.id)}>
                            {item.checked ?
                                ...
                                <Square size={20} color={C.textLight} />
                            }
                        </AnimatedTouchable>
                        <Text style={[styles.itemText, { color: C.textDark }, item.checked && styles.itemTextChecked]}>
                            {item.text}
                        </Text>
                        <AnimatedTouchable onPress={() => onDeleteItem(checklist.id, item.id)}>
                            <Trash2 size={16} color={C.border} />
                        </AnimatedTouchable>
                    </View>
                ))}
            </View>
            ...
            <View style={styles.addItemRow}>
                <TextInput
                    style={[styles.addItemInput, { color: C.textDark }]}
                    value={newItemText}
