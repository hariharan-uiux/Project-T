            ...
            >
                ...
                <View style={[styles.header, { borderBottomColor: C.border }]}>
                    <View style={styles.titleContainer}>
                        ...
                        {isEditingTitle ? (
                            <TextInput
                                style={[styles.titleInput, { color: C.black, borderBottomColor: COLORS.primary }]}
                                value={titleText}
                            ...
                            <Text style={[styles.title, { color: C.black }]}>{checklist.title}</Text>
                        )}
                    </View>
                    ...
                    {[...(checklist.items || [])].sort((a, b) => {
                        ...
                        <View key={item.id} style={[styles.itemRow, { borderBottomColor: C.border }]}>
                            ...
                            {editingItemId === item.id ? (
                                <TextInput
                                    style={[styles.itemText, { color: C.black, borderBottomWidth: 1, borderBottomColor: COLORS.primary }]}
                                ...
                                <Text
                                    style={[styles.itemText, { color: C.black }, item.checked && styles.itemTextChecked]}
                            ...
                            <AnimatedTouchable onPress={() => onDeleteItem(checklist.id, item.id)} style={[styles.actionBtnIcon, { backgroundColor: C.white, borderColor: C.border }]}>
                                <XIcon size={24} color={C.textLight} strokeWidth={2.5} />
                            </AnimatedTouchable>
                        </View>
                    ))}
                    {showAddInput && (
                        <View style={[styles.itemRow, { borderBottomColor: C.border }]}>
                            <Circle size={26} color={C.textLight} strokeWidth={2} style={styles.checkboxWrapper} />
                            <TextInput
                                style={[styles.addItemInputInner, { color: C.black }]}
                                value={newItemText}
                            ...
                            {newItemText.length > 0 ? (
                                <AnimatedTouchable onPress={handleAddItem} style={[styles.actionBtnIcon, { backgroundColor: C.white, borderColor: C.border }]}>
                                    <Plus size={24} color={COLORS.primary} strokeWidth={2.5} />
                                </AnimatedTouchable>
                            ) : (
                                <AnimatedTouchable onPress={() => setShowAddInput(false)} style={[styles.actionBtnIcon, { backgroundColor: C.white, borderColor: C.border }]}>
                                    <XIcon size={24} color={C.textLight} strokeWidth={2.5} />
                                </AnimatedTouchable>
                            )}
                        </View>
